...
//</editor-fold>
    ...
    public boolean isQty(String input) {
        try {
...
//</editor-fold>
    ...
    public boolean chkPayments(String price) {
        boolean valid = false;

        if ("0.00".equals(price) && chkPrice(price)) {
            return valid;
        } else {
            return valid = true;
        }
...
//</editor-fold>
    ...
    public static boolean doesDate1GraterThanDate2(Date toCheckFrom,
            Date toCheckTo, String timeFrom, String timeTo) {
        try {
            if (toCheckFrom == null || toCheckTo == null || timeFrom == null
                    || timeTo == null) {
                return false;
            } else {
                if (toCheckFrom.equals(toCheckTo) && (toCheckFrom.equals(
                        toCheckTo)
...
//</editor-fold>
    ...
    public boolean validName(ListView listView) {
        boolean valid = false;
        ...
        try {
            if (list.size() != 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (list.get(i).length() > 0 && list.get(i).length() < 300) {
                        valid = true;
                    } else {
...
//</editor-fold>
    public boolean validQty(String No, Double qty) {
        ...
        matched_Number = matcher_Number.matches();       // Result of the matcher is returned as a boolean for Single Name
        try {
            return matched_Number && qty >= Double.parseDouble(No) && 0 < Double.parseDouble(No);
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean validQtyViseVersa(String No, Double qty, Double qty2) {
        boolean matched_Number = false;
        String number = No.trim();
        ...
        try {
            ...
            return matched_Number && (qty < val) && (0 < val) && (qty2 >= val);
        } catch (Exception e) {
            return false;
        }
...
//</editor-fold>
    public boolean overlapNumber(String No1, String No2) {
        ...
        if ((!number1.isEmpty() && !number2.isEmpty()) && (Integer.parseInt(
    ...
    public boolean overlapNumberDouble(String No1, String No2) {
        String number1 = No1.trim();
        String number2 = No2.trim();
        try {
            boolean matchedNumber = validNumber(No2);

            if (matchedNumber) {
                if ((!number1.isEmpty() && !number2.isEmpty()) && (Double.
                        parseDouble(number1) <= Double.parseDouble(number2))) {
    ...
    public boolean overlapNumberDoublePayment(String No1, String No2) {
        String number1 = No1.trim();
        String number2 = No2.trim();
        try {
            boolean matchedNumber = validPositiveDoubleAmount(No2);
            ...
            if (matchedNumber) {
                ...
                } else if ((number1.isEmpty() && number2.isEmpty()) && (Double.
                        parseDouble(number1) == Double.parseDouble(number2))) {
                    return true;
                } else {
    ...
    public boolean overlapNumberDoubleInAdditionalFood(String No1, String No2) {
        String number1 = No1.trim();
        String number2 = No2.trim();

//        String number1_InString = No1.trim();
//        String number2_InString = No2.trim();
//        
//        double number1_InDouble = Double.parseDouble(number1_InString);
//        double number2_InDouble = Double.parseDouble(number2_InString);
//        
//        
//            System.out.println("Number 01 in String: "+number1_InString);
//            System.out.println("Number 02 in String: "+number1_InString);
//            System.out.println("Number 01 in Double: "+number1_InDouble);
//            System.out.println("Number 02 in Double: "+number2_InDouble);
        try {
            boolean matchedNumber = validNumber(No2);
    ...
    //<editor-fold defaultstate="collapsed" desc="validNameWithoutNumber">
    //========================================================================================
    //validate Profession.parameter a string
    //========================================================================================
//</editor-fold>
    public boolean validNameWithoutNumber(String name) {
        ...
        try {
            if (!name.isEmpty()) {
                ...
                Matcher matcher_single_name = pattern_single_name.matcher(
                        name.trim());   // Pattern Matcher where the Single Name argument is checked with the given pattern
                Matcher matcher_multiple_name = pattern_multiple_name.matcher(
                        name.trim());  // Pattern Matcher where the Multiple Names argument is checked with the given pattern
...
//</editor-fold>
    ...
    public boolean validPhoneNo(ObservableList<String> list) {
        boolean valid = true;
        ...
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                ...
                Matcher matcher_Number = pattern_Number.matcher(list.get(i));   // Pattern Matcher where the Single Name argument is checked with the given pattern

                boolean matched_Number = matcher_Number.matches();       // Result of the matcher is returned as a boolean for Single Name
                System.out.println("num " + matched_Number);
                if (matched_Number == true && ((list.get(i).length() == 10)
                        || (list.get(i).length() == 13))) {
                    if (validNumber(list.get(i).trim())) {
                        valid = true;
...
//</editor-fold>
    ...
    public boolean validNonZeroPositiveInteger(String amount) {
        boolean valid = false;
        int value = 0;
        try {
    ...
    public boolean isAmountSettled(String amount) {
        boolean valid = false;
        double value = 0.0;
        try {
            value = Double.parseDouble(amount);
    ...
    public boolean isValidCurrency(String acctualCurrency, String getCurrency,
            String acctualPaymethod, String getPaymethod) {

        boolean valid = true;
        if (acctualCurrency.equals(getCurrency) && acctualPaymethod.equals(
                getPaymethod)) {
            valid = false;
        }
        return valid;
    ...
    //========================================================================================
//</editor-fold>
    public boolean validTableViewSize(TableView tableView) {
        boolean valid = false;
//        try {
